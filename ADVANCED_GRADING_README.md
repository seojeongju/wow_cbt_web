...
모의고사 출제 시 과목별 과락 설정 및 전체 평균점수 기준 설정 기능이 추가되었습니다.
...
- subject_min_scores: TEXT -- JSON: {"subjectId": minScore, ...}
- use_subject_min_score: INTEGER -- 과목별 과락 사용 여부 (0/1)
```
...
### Exam Results 테이블
```sql
- subject_scores: TEXT -- JSON: {"subjectId": score, ...}
...
→ 전체 60점 이상 + 과목 평균 60점 이상이면 합격

### 시나리오 3: 엄격한 기준
- 전체 합격 점수: 70점
- 평균 점수 기준: 60점 (사용)
- 과목별 과락: 각 과목 40점 이상 (사용)

→ 세 가지 조건 모두 충족해야 합격
- 예: 과목 A(90점), 과목 B(30점) = 평균 60점, 전체 60점
  - 결과: 불합격 (과목 B 과락)
