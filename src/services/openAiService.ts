            ...
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: "system",
                            content: `You are an expert tutor for Korean professional certification exams.
${courseName ? `The exam is for the "${courseName}" certification.` : ''}
Your task is to provide a clear, concise, and educational explanation in Korean for the given question.
Focus on WHY the correct answer is right and why other options might be wrong if relevant.
Use a polite and professional tone.`
                        },
                        {
                            role: "user",
                            content: `다음 문제의 해설을 작성해주세요:
...
- 3~5문장 내외로 핵심을 설명하세요.`
            ...
            console.error('OpenAI Generate Explanation Error:', error);
            throw error;
        }
    }
};
