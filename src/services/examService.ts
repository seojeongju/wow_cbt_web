...
export const ExamService = {
    ...
    submitExamResult: async (examId: string, answers: { [key: string]: number | string }, score: number) => {
        ...
        const overallPercentage = Math.round(score * (100 / (totalQuestions || 1)));
        
        // 🔄 Advanced grading: Category-based scores
        const categoryGroups: { [cat: string]: { total: number; correct: number } } = {};
        exam.questions.forEach(q => {
            const cat = q.category || '일반';
            if (!categoryGroups[cat]) categoryGroups[cat] = { total: 0, correct: 0 };
        ...
        const catPercentageList: number[] = [];
        Object.entries(categoryGroups).forEach(([cat, stats]) => {
            const pct = Math.round(stats.correct * (100 / (stats.total || 1)));
            categoryScores[cat] = pct;
            catPercentageList.push(pct);
        });

        const averageScore = catPercentageList.length > 0 
            ? Math.round(catPercentageList.reduce((a, b) => a + b, 0) / catPercentageList.length)
            : overallPercentage;
        ...
        if (overallPercentage < passScore) {
            failReasons.push(`전체 점수 미달 (${overallPercentage}점 < 기준 ${passScore}점)`);
        }

        // 2. Average Score check
        if (exam.useAverageScore && exam.averagePassScore !== undefined && exam.averagePassScore !== null) {
            if (averageScore < exam.averagePassScore) {
                failReasons.push(`평균 점수 미달 (${averageScore}점 < 기준 ${exam.averagePassScore}점)`);
            }
        }

        // 3. Category Minimum Score check (과락)
        if (exam.useCategoryMinScore && exam.categoryMinScores) {
            Object.entries(exam.categoryMinScores).forEach(([cat, minScore]) => {
                const catScore = categoryScores[cat] || 0;
                if (catScore < minScore) {
                    failReasons.push(`${cat} 과락 (${catScore}점 < 기준 ${minScore}점)`);
                }
            });
        }
