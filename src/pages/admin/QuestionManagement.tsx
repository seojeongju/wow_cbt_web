...
import { ChevronLeft, ChevronRight, Plus, Trash2, FileUp, Folder, FileText, LayoutGrid, BookOpen, Edit2, Image as ImageIcon, Settings, BrainCircuit, Sparkles, X, CheckCircle, Printer } from 'lucide-react';
import { Question } from '../../types';
import { ExamService } from '../../services/examService';
import { CategoryService } from '../../services/categoryService';
import { CourseService } from '../../services/courseService';
import { SubjectService } from '../../services/subjectService'; // Added
import { SettingsService } from '../../services/settingsService'; // Added ⭐️
...
export const QuestionManagement = () => {
    ...
    const loadInitialData = async () => {
        ...
        // ⭐️ AI API Key Sync from DB
        try {
            const systemSettings = await SettingsService.getSettings();
            if (systemSettings.openaiApiKey) {
                // DB에 키가 있으면 localStorage보다 우선하거나 보완함
                const localKey = localStorage.getItem('openai_api_key');
                if (!localKey || localKey !== systemSettings.openaiApiKey) {
                    localStorage.setItem('openai_api_key', systemSettings.openaiApiKey);
                    setApiKey(systemSettings.openaiApiKey);
                }
            }
        } catch (err) {
            console.error('Failed to sync AI settings:', err);
        }
    ...
    */
    ...
    const handleGenerateSimilar = async (question: Question) => {
        if (!apiKey) {
            alert('시스템 설정에서 OpenAI API Key를 먼저 설정해주세요.');
            return;
        }
    ...
    const handleGenerateSingleExplanation = async (question: Question) => {
        ...
        if (question.explanation && !confirm('이미 해설이 존재합니다. AI가 새로 생성한 내용으로 덮어쓰시겠습니까?')) {
            return;
        }

        setIsAiProcessing(true);
        try {
            const explanation = await OpenAIService.generateSingleExplanation(
                { text: question.text, options: question.options, correctAnswer: question.correctAnswer as number },
                apiKey,
                selectedCourse || undefined
            );

            if (explanation) {
                const updatedQuestion = { ...question, explanation };
                await ExamService.updateQuestionInExam(selectedExamId, updatedQuestion);
                alert('AI 해설이 생성 및 저장되었습니다.');
                loadQuestions(selectedExamId);
            }
        } catch (error: any) {
            console.error('AI 해설 생성 오류:', error);
            alert('해설 생성 실패: ' + error.message);
        } finally {
            setIsAiProcessing(false);
        }
    };
                                                    ...
                                                    return (
                                                        <div key={q.id} className="glass-card" style={{
                                                            ...
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                                ...
                                                                <button
                                                                    onClick={() => handleGenerateSingleExplanation(q)}
                                                                    className="btn btn-secondary"
                                                                    style={{ height: 'fit-content', padding: '0.5rem', color: '#10b981' }}
                                                                    title="AI 해설 자동 생성"
